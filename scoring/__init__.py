...
class ensemble_scorer(object):
    def __init__(self, model_instances, descriptor_generator_instances):
        ...
        self.descriptor_generators = descriptor_generator_instances
        if type(descriptor_generator_instances) is list:
    ...
    def fit(self, ligands, target):
        if self.single_descriptor:
            self.train_descs = self.descriptor_generators.build(ligands)
        else:
            self.train_descs = [desc_gen.build(ligands) for desc_gen in self.descriptor_generators]
        self.train_target = target
        ...
        if self.single_model and self.single_descriptor:
            ...
            return [model.fit(self.train_descs[n],target) for n, model in enumerate(self.models)]
    
    def predict(self, ligands):
        if self.single_model and self.single_descriptor:
            descs = self.descriptor_generator.build(ligands)
            return self.models.predict(descs)
        elif self.single_model and not self.single_descriptor:
            return [self.models.predict(descs) for desc in self.train_descs]
        else:
            descs = [desc_gen.build(ligands) for desc_gen in self.descriptor_generators]
            return [model.predict(descs[n],target) for n, model in enumerate(self.models)]
    
    def score(self, ligands, target):
        if self.single_model and self.single_descriptor:
            descs = self.descriptor_generator.build(ligands)
            return self.models.score(descs)
        elif self.single_model and not self.single_descriptor:
            return [self.models.score(descs) for desc in self.train_descs]
        else:
            descs = [desc_gen.build(ligands) for desc_gen in self.descriptor_generators]
            return [model.score(descs[n],target) for n, model in enumerate(self.models)]
