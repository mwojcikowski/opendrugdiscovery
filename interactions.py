...
class mol:
    ...
    def dict_numpy(self):
        ...
        res_mask_alpha = np.where(((phi > -145) & (phi < -35) & (psi > -70) & (psi < 50)))
        res_dict['isalpha'][res_mask_alpha] = True
        mol_dict['isalpha'][np.in1d(mol_dict['resid'], res_dict[res_mask_alpha]['id'])] = True
        res_mask_beta = np.where(((phi >= -180) & (phi < -40) & (psi <= 180) & (psi > 90)) | ((phi >= -180) & (phi < -70) & (psi <= -165)))
        res_dict['isbeta'][res_mask_beta] = True
        mol_dict['isbeta'][np.in1d(mol_dict['resid'], res_dict[res_mask_beta]['id'])] = True

        # Rings
        for ring in molecule.sssr:
            if ring.IsAromatic():
                path = np.array(ring._path) # path oth atom ID's (1-n)
                coords = mol_dict[path-1]['coords']
                centroid = coords.mean(axis=0)
                # get vector perpendicular to ring
                vector = np.cross(mol_dict[path-1]['coords'] - mol_dict[np.hstack((path[1:],path[0]))-1]['coords'],mol_dict[np.hstack((path[1:],path[0:1]))-1]['coords'] - mol_dict[np.hstack((path[2:],path[0:2]))-1]['coords']).mean(axis=0)
